...
def createPlaidPayrollPdf(pdf, width, height, item):
    ...
    employerTownAndZip = item.payroll_income[0].pay_stubs[0].employer.address.city + ", " + \
        item.payroll_income[0].pay_stubs[0].employer.address.region + " " + \
        item.payroll_income[0].pay_stubs[0].employer.address.postal_code
    ...
    employeeTownAndZip = item.payroll_income[0].pay_stubs[0].employee.address.city + ", " + \
        item.payroll_income[0].pay_stubs[0].employee.address.region + " " + \
        item.payroll_income[0].pay_stubs[0].employee.address.postal_code
    ...
    currentTotalEarnings = currentSalary + \
        currentSupplementalBonus + currentOvertime + currentCommission
    ...
    format_plaid_income(pdf, width, height, employerName, employerStreet, employerTownAndZip,
                        ...
                        str(grossEarnings), str(totalTaxesAndDeductions), str(
                            netPay), bankName, str(bankMask),
                        str(currentSalary), str(ytdSalary), str(currentSupplementalBonus), str(ytdSupplementalBonus), str(currentOvertime), str(
                            ytdOvertime), str(currentCommission), str(ytdCommission), str(currentTotalEarnings), str(ytdTotalEarnings),
...
def format_plaid_income(pdf, width, height, employerName, employerStreet, employerTownAndZip,
    ...
    pdf.drawString((width - headerStringWidth)/2, height -
                   (heightTracker*inch), headerString)
    heightTracker += 0.1
    pdf.line((width/2)-(headerStringWidth/2), height-(inch*heightTracker),
             (width/2)+(headerStringWidth/2), height-(inch*heightTracker))
    ...
    pdf.setFont("Helvetica-Bold", 10)
    heightTracker += 0.7
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Pay Period Start: ")
    pdf.setFont("Helvetica", 10)
    pdf.drawRightString(width-(inch*1.1), height -
                        (heightTracker*inch), payPeriodStart)
    ...
    pdf.setFont("Helvetica", 10)
    pdf.drawRightString(width-(inch*1.1), height -
                        (heightTracker*inch), payPeriodEnd)
    ...
    pdf.setFont("Helvetica-Bold", 10)
    heightTracker += 0.1
    pdf.rect(0.9*inch, height-(heightTracker*inch),
             (width-(1.8*inch)), inch*0.7)

    # Draw Payment Summary Information
    pdf.setFont("Helvetica-Bold", 10)
    heightTracker += 0.2
    paymentSummaryStringWidth = stringWidth(
        "Payment Summary", "Helvetica-Bold", 10)
    pdf.drawString((width - paymentSummaryStringWidth)/2,
                   height-(heightTracker*inch), "Payment Summary")

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch), "Gross Earnings: ")
    pdf.setFont("Helvetica", 10)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), grossEarnings)
    ...
    pdf.setFont("Helvetica", 10)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), totalTaxesAndDeductions)
    ...
    pdf.setFont("Helvetica", 10)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), bankName + "-" + bankMask)
    ...
    pdf.setFont("Helvetica-Bold", 10)
    heightTracker += 0.2
    grossEarningsSummaryWidth = stringWidth(
        "Gross Earnings", "Helvetica-Bold", 10)
    pdf.drawString((width - grossEarningsSummaryWidth)/2,
                   height-(heightTracker*inch), "Gross Earnings")

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch), "Description")
    currentStringWidth = stringWidth("Current", "Helvetica-Bold", 10)
    pdf.drawString((width-currentStringWidth)/2, height -
                   (heightTracker*inch), "Current")
    ...
    pdf.drawString((width-currentSalaryWidth)/2, height -
                   (heightTracker*inch), currentSalary)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), ytdSalary)

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Supplemental pay: Bonus")
    currentSupplementalBonusWidth = stringWidth(
        currentSupplementalBonus, "Helvetica", 10)
    pdf.drawString((width-currentSupplementalBonusWidth)/2,
                   height-(heightTracker*inch), currentSupplementalBonus)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), ytdSupplementalBonus)

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Supplemental Pay: Overtime")
    currentSupplementalOvertimeWidth = stringWidth(
        currentOvertime, "Helvetica", 10)
    pdf.drawString((width-currentSupplementalOvertimeWidth)/2,
                   height-(heightTracker*inch), currentOvertime)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), ytdOvertime)

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Supplemental Pay: Commission")
    currentCommissionWidth = stringWidth(currentCommission, "Helvetica", 10)
    pdf.drawString((width-currentCommissionWidth)/2, height -
                   (heightTracker*inch), currentCommission)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), ytdCommission)

    heightTracker += 0.1
    pdf.line((inch*1.1), height-(heightTracker*inch),
             width-(inch*1.1), height-(heightTracker*inch))

    pdf.setFont("Helvetica-Bold", 10)
    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Gross Earnings Totals")
    currentTotalWidth = stringWidth(currentTotalEarnings, "Helvetica-Bold", 10)
    pdf.drawString((width-currentTotalWidth)/2, height -
                   (heightTracker*inch), currentTotalEarnings)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), ytdTotalEarnings)
    ...
    pdf.drawString((width - taxesWithheldWidth)/2, height -
                   (heightTracker*inch), "Taxes Withheld")
    ...
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Federal Income Tax")
    currentFederalIncomeTaxWidth = stringWidth(
        currentFederalIncomeTax, "Helvetica", 10)
    pdf.drawString((width-currentFederalIncomeTaxWidth)/2,
                   height-(heightTracker*inch), currentFederalIncomeTax)

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch), "Social Security")
    currentSocialSecurityWidth = stringWidth(
        currentSocialSecurity, "Helvetica", 10)
    pdf.drawString((width-currentSocialSecurityWidth)/2, height -
                   (heightTracker*inch), currentSocialSecurity)

    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch), "Medicare")
    currentMedicareWidth = stringWidth(currentMedicare, "Helvetica", 10)
    pdf.drawString((width-currentMedicareWidth)/2, height -
                   (heightTracker*inch), currentMedicare)
    ...
    pdf.drawString((width-currentStateTaxWidth)/2, height -
                   (heightTracker*inch), currentStateTax)
    ...
    heightTracker += 0.2
    pdf.drawString((inch*1.1), height-(heightTracker*inch),
                   "Taxes Withheld Totals")
    currentTotalTaxesWidth = stringWidth(
        currentTotalTaxWithheld, "Helvetica-Bold", 10)
    pdf.drawString((width-currentTotalTaxesWidth)/2, height -
                   (heightTracker*inch), currentTotalTaxWithheld)
    pdf.drawRightString((width-(inch*1.1)), height -
                        (heightTracker*inch), ytdTotalTaxWithheld)
...
def send_email(sender, recipient, plaidResponseObject):
    ...
    for item in plaidResponseObject.items:
        ...
        plaidFinancialInformationPdf = canvas.Canvas(
            "/tmp/" + pdfName, pagesize=letter)
        width, height = letter
        createPlaidPayrollPdf(
            plaidFinancialInformationPdf, width, height, item)
